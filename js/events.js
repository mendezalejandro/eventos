    ...
    data._embedded.classifications.forEach((categoria) => {
      if (categoria.segment !== undefined) {
        $("#inputCategoria").append(
          $("<option>").val(categoria.segment.id).text(categoria.segment.name)
        );
      }
    });
  });
...
function loadFechas() {
  ...
  for (var mes = 0; mes < 12; mes++) {
    ...
    const primerDia = fechaReferencia
      .startOf("month")
      .format("yyyy-MM-DDTHH:mm:ss");
    const ultimoDia = fechaReferencia
      .endOf("month")
      .format("yyyy-MM-DDTHH:mm:ss");

    $("#inputFecha").append(
      $("<option>")
        .val(primerDia + "," + ultimoDia)
        .text(meses[fechaReferencia.month()] + " " + fechaReferencia.year())
    );
  }
}
...
function loadEventos(data) {
  ...
  // si no tengo info entonces muestro un msj en la pantalla
  if (data._embedded === undefined) {
    $("<h3>No se encontrarón eventos para los filtros seleccionados</h3>").appendTo($("#cards"));
  } else {
    ...
    data._embedded.events.forEach((evento) => {
      ...
      evento.images.forEach((image) => {
        if (image.width < 1024) imagen = image.url;
        return false;
      });

      // construyo el html de la tarjeta del evento
      var cardHTML = buildCard(
        evento.id,
        evento.name,
        imagen,
        evento._embedded.venues[0].city.name,
        moment(evento.dates.start.localDate).format('DD/MM/YYYY'),
...
function loadPagination() {
  ...
  if (pageInfo === undefined) {
    ...
    $("#infoPag").text(
      "Pagina " + (pageInfo.number + 1) + " de " + pageInfo.totalPages
    );
  }
}
